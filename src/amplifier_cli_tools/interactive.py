...
Example:
    >>> from amplifier_cli_tools.interactive import run_checklist
    >>> items = [
    ...     ChecklistItem("projects", "Session transcripts and history", True),
    ...     ChecklistItem("settings", "User configuration", True),
    ...     ChecklistItem("cache", "Downloaded bundles", False),
    ... ]
    >>> selected = run_checklist(items, title="Select items to preserve")
    >>> print(selected)  # {"projects", "settings"}
"""
